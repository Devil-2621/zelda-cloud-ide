...
export const createScrapeUrlsTool = ({ internalKey, messageId }: ScrapeUrlsToolOptions) => {
  return createTool({
    name: "scrapeUrls",
    description:
    ...
    handler: async (params, { step: toolStep }) => {
      ...
      try {
        return await toolStep?.run("scrape-urls", async () => {
          await convex.mutation(api.system.updateMessageCurrentStatus, {
            internalKey,
            messageId,
            currentStatus: `Fetching ${urls.length === 1 ? urls[0] : `${urls.length} URLs`}...`,
          });
